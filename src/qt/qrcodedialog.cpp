...
{
    QString ret = QString("laughingmancoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LMC unit input in lnReqAmount, we generate the URI with LMC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LMC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
