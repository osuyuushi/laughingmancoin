...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uLMC
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 8200000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 8200000);
...
void ThreadScriptCheck() {
    RenameThread("laughingmancoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1396528998, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x068fd853 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x06, 0x8f, 0xd8, 0x53 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LaughingmancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LaughingmancoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("LaughingmancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("laughingmancoin-miner");
    ...
    try { loop {
        ...
        printf("Running LaughingmancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LaughingmancoinMiner terminated\n");
        throw;
    }
}
